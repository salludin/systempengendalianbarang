...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE sumber_dana SET nm_dana=%s,jumlah=%s WHERE id_dana=%s",
...
$colname_rs_edit = "-1";
if (isset($_GET['id_dana'])) {
  $colname_rs_edit = $_GET['id_dana'];
}
mysql_select_db($database_koneksi, $koneksi);
$query_rs_edit = sprintf("SELECT * FROM sumber_dana WHERE id_dana = %s", GetSQLValueString($colname_rs_edit, "int"));
...
<div class="grid_12">
  <div class="block-border">
    ...
    <form action="<?php echo $editFormAction; ?>" name="form"  method="POST" enctype="multipart/form-data" class="block-content form">
      <div class="_50">
        <p>
          <label for="textarea">Nama Bidang Usaha</label><input id="textfield" name="nm_dana" class="required" type="text" value="<?php echo $row_rs_edit['nm_dana']; ?>" />
        </p>
        <p>
          <label for="textarea2">Jumlah</label>
          <input id="textfield2" name="jumlah" class="required" type="text" value="<?php echo $row_rs_edit['jumlah']; ?>" />
        </p>
      </div>
      <div class="clear"></div>
      ...
      <input name="id_dana" type="hidden" id="id_dana" value="<?php echo $row_rs_edit['id_dana']; ?>" />
