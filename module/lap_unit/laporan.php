...
$objPHPExcel->getProperties()->setCreator("Fauzan Azis")
							 ...
							 ->setTitle("Data Unit Kerja")
							 ->setSubject("Data Unit Kerja")
							 ->setDescription("Laporan Data Unit Kerja Universitas Muhammadiyah Palembang")
							 ->setKeywords("Data Unit Kerja")
							 ->setCategory("Data Unit Kerja");
...
$objPHPExcel->getActiveSheet()->setCellValue('A9', "NO")
							  ->setCellValue('B9', "KODE UNIT ")
							  ->setCellValue('C9', "NAMA UNIT")
							  ->setCellValue('D9', "KETERANGAN");

// koneksi database
include ("../../Connections/koneksi.php"); 
mysql_select_db($database_koneksi,$koneksi);
// query database
$SQL = mysql_query("select * from unit_kerja order by kode_unit ASC");
...
while($row = mysql_fetch_array($SQL, MYSQL_ASSOC)){
	$no++;
	$row_array['no'] 		  		= $no;
	$row_array['kode_unit'] 	  	= $row['kode_unit'];
	$row_array['nm_unit']    	    = $row['nm_unit'];
...
// Set title row bold;
$objPHPExcel->getActiveSheet()->getStyle('A9:D9')->getFont()->setBold(true);
// Set fills
$objPHPExcel->getActiveSheet()->getStyle('A9:D9')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
$objPHPExcel->getActiveSheet()->getStyle('A9:D9')->getFill()->getStartColor()->setARGB('FF808080');
...
$objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle1, "A9:D$nox");
...
$objPHPExcel->getActiveSheet()->setCellValue('A2', "DAFTAR UNIT KERJA");
...
$objPHPExcel->getActiveSheet()->getStyle('A2:D5')->getFont()->setName('Arial');
$objPHPExcel->getActiveSheet()->getStyle('A2:D5')->getFont()->setSize(14);
$objPHPExcel->getActiveSheet()->getStyle('A2:D5')->getFont()->setBold(true);
$objPHPExcel->getActiveSheet()->getStyle('A3:D5')->getFont()->setSize(13);
...
header('Content-Disposition: attachment;filename="DATA_UNIT_KERJA"'.date("d-F-Y").'".xlsx"');
