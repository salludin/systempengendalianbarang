...
$objPHPExcel->getActiveSheet()->setCellValue('A9', "NO")
							  ->setCellValue('B9', "KODE PENGADAAN")
							  ->setCellValue('C9', "NAMA BARANG")
							  ->setCellValue('D9', "SUPLIER")
							  ->setCellValue('E9', "TANGGAL BELI")
							  ->setCellValue('F9', "JUMLAH")
							  ->setCellValue('G9', "HARGA")
							  ->setCellValue('H9', "SUB TOTAL");
...
// query database
$SQL = mysql_query("select *,jumlah*harga_beli as subtotal from tampil_pengadaan WHERE (tgl_beli >= '$_POST[tgl]') AND (tgl_beli <= '$_POST[tgl1]') ");
$SQL_total = mysql_query("select SUM(harga_beli * jumlah) as total from tampil_pengadaan WHERE (tgl_beli >= '$_POST[tgl]') AND (tgl_beli <= '$_POST[tgl1]') ");

$qry_total = mysql_fetch_assoc($SQL_total);
$total = $qry_total['total'];
// jumlah data
...
while($row = mysql_fetch_array($SQL, MYSQL_ASSOC)){
	$no++;
	$row_array['no'] 		  			= $no;
	$row_array['kode_pengadaan'] 	  	= $row['kode_pengadaan'];
	$row_array['nm_barang']    			= $row['nm_barang'];
	$row_array['nm_suplier'] 	  		= $row['nm_suplier'];
	$row_array['tgl_beli'] 			    = $row['tgl_beli'];
	$row_array['jumlah'] 			    = $row['jumlah'];
	$row_array['harga_beli'] 	  		= $row['harga_beli'];
...
$objPHPExcel->getActiveSheet()->setCellValue('A2', "DAFTAR DATA PEGADAAN INVENTARIS");
...
$objPHPExcel->getActiveSheet()->setCellValue('A'.$baris, "JUMLAH TOTAL");
...
header('Content-Disposition: attachment;filename="DATA_PENGADAAN"'.date("d-F-Y").'".xlsx"');
