...
$(document).ready(function() {
	$("#jumlah-").keyup(function(){
		...
		if (jumlah > sisa_jml) {
			...
			$("#txt_mainten_cari").keyup(function(){
				var cari	= $("#txt_mainten_cari").val();								  
				$.ajax({
					type	: "POST",
					url		: "module/mainten_inventaris/tampil_data_barang.php",
					data	: "cari="+cari,
					timeout	: 3000,
					beforeSend	: function(){		
...
function hapus_data(ID) {
	...
	$('#input_mainten').submit(function(e){
                    e.preventDefault();
                    $.ajax({
                        'type': 'POST',
                        'url': 'module/mainten_inventaris/simpan.php',
                        'data': $(this).serialize(),
						'timeout'	: 3000,
						'beforeSend'	: function(){		
						    $('#tampil').show();
