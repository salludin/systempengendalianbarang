...
$objPHPExcel->getProperties()->setCreator("Fauzan Azis")
							 ...
							 ->setTitle("Data maintenance")
							 ->setSubject("Data maintenance")
							 ->setDescription("Laporan Data maintenance ")
							 ->setKeywords("Data maintenance")
							 ->setCategory("Data maintenance");
...
$objPHPExcel->getActiveSheet()->setCellValue('A9', "NO")
							  ->setCellValue('B9', "ID")
							  ->setCellValue('C9', "KODE INVENTARISASI")
							  ->setCellValue('D9', "TANGGAL SERVIS")
							  ->setCellValue('E9', "TGL SERV. BERIKUTNYA")
							  ->setCellValue('F9', "TEMPAT SERVIS")
							  ->setCellValue('G9', "KELUHAN")
							  ->setCellValue('H9', "KETERANGAN")
...
// query database
$SQL = mysql_query("select * from maintenance order by id_pemeliharaan ASC ");
...
while($row = mysql_fetch_array($SQL, MYSQL_ASSOC)){
	$no++;
	$row_array['no'] 		  			= $no;
	$row_array['id_pemeliharaan'] 	  	= $row['id_pemeliharaan'];
	$row_array['kode_inventarisasi']   	= $row['kode_inventarisasi'];
	$row_array['tgl_servis']    		= $row['tgl_servis'];
	$row_array['tgl_servis_berikutnya'] = $row['tgl_servis_berikutnya'];
	$row_array['tempat_servis'] 		= $row['tempat_servis'];
	$row_array['keluhan'] 	  			= $row['keluhan'];
	$row_array['keterangan_pem'] 	  	= $row['keterangan_pem'];
...
$objPHPExcel->getActiveSheet()->setCellValue('A2', "DAFTAR DATA PEMELIHARAAN INVENTARIS");
...
header('Content-Disposition: attachment;filename="DATA_JADWAL_PEMELIHARAAN"'.date("d-F-Y").'".xlsx"');
