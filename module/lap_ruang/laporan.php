...
$objPHPExcel->getProperties()->setCreator("Fauzan Azis")
							 ...
							 ->setSubject("Data Ruangan")
							 ->setDescription("Laporan Data Ruangan Universitas Muhammadiyah Palembang")
							 ->setKeywords("Data Ruangan")
							 ->setCategory("Data Ruangan");
...
$objPHPExcel->getActiveSheet()->setCellValue('A9', "NO")
							  ->setCellValue('B9', "KODE RUANGAN")
							  ->setCellValue('C9', "NAMA RUANGAN")
...
// query database
$SQL = mysql_query("select * from ruangan order by kode_ruangan ASC");
...
while($row = mysql_fetch_array($SQL, MYSQL_ASSOC)){
	$no++;
	$row_array['no'] 		  		= $no;
	$row_array['kode_ruangan'] 	  	= $row['kode_ruangan'];
	$row_array['nm_ruangan']    	= $row['nm_ruangan'];
...
$objPHPExcel->getActiveSheet()->setCellValue('A2', "DAFTAR DATA RUANGAN");
...
header('Content-Disposition: attachment;filename="DATA_RUANGAN"'.date("d-F-Y").'".xlsx"');
