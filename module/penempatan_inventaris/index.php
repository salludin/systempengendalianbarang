...
$query_rs_data = "SELECT kode_inventarisasi,nm_barang,nm_ruangan,nm_unit,jumlah,baik,rusak,kondisi,nm_status,inventarisasi.tgl_posting,inventarisasi.user_posting FROM inventarisasi LEFT JOIN aset ON inventarisasi.kode_barang = aset.kode_barang LEFT JOIN ruangan ON inventarisasi.kode_ruangan = ruangan.kode_ruangan LEFT JOIN unit_kerja ON inventarisasi.kode_unit = unit_kerja.kode_unit LEFT JOIN status ON inventarisasi.status = status.status ORDER BY kode_inventarisasi ASC";
         ...
         <?php $no = 1;?>
          <?php do { ?>
            <tr class=gradeX>
              ...
              <td  width="50"><?php echo $row_rs_data['baik']; ?></td>
              <td  width="50"><?php echo $row_rs_data['rusak']; ?></td>
